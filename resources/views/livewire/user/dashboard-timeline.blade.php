...
.ph-avatar { width:42px; height:42px; border-radius:50%; object-fit:cover; flex-shrink:0; border:1px solid var(--ph-border); }
...
/* Ready state — file staged, not yet on Cloudinary */
.ph-vready {
    margin:0 16px 10px; background:#fff8e1; border:1px solid #ffd54f;
    border-radius:10px; padding:16px;
}
.ph-vready-title { font-weight:700; font-size:14px; color:#5d4037; margin-bottom:4px; }
.ph-vready-sub   { font-size:12px; color:#795548; margin:0 0 12px; }
.ph-upload-btn {
    background:#f02849; color:#fff; border:none; border-radius:8px;
    padding:10px 20px; font-size:14px; font-weight:700; cursor:pointer;
    font-family:inherit; display:inline-flex; align-items:center; gap:8px;
    transition:background .15s;
}
.ph-upload-btn:hover { background:#c0212a; }
.ph-upload-btn:disabled { background:#bcc0c4; cursor:not-allowed; }
.ph-change-link {
    margin-left:12px; font-size:12px; color:var(--ph-sub);
    ...
    <div class="col-md-8">
        ...
        <div class="ph-card">
            ...
            <div class="ph-ref-row">
                <input type="text" value="{{ url('/reg?referral_code='.auth()->user()->referral_code) }}" readonly>
                <button type="button" onclick="copyReferralLink()">
                    <i class="fa fa-copy me-1"></i>Copy
                </button>
            </div>
            <a href="{{ url('referral/list') }}" style="color:var(--ph-blue);font-size:13px;font-weight:600;text-decoration:none">
                View all referrals →
            </a>
        </div>
        ...
        <div class="ph-composer"
            ...
            <div class="ph-top">
                ...
                <div class="ph-textarea-wrap">
                    <textarea
                        ...
                        class="ph-textarea"
                        :placeholder="mode==='video' ? 'Add a caption for your video…' : `What\'s on your mind, {{ auth()->user()->name }}?`"
                        rows="2"
            ...
            {{-- Image previews --}}
            @if(in_array($userLevel,['Creator','Influencer']))
                <div class="ph-media-preview" x-show="mode==='image' && previews.length" x-cloak>
                    <div class="ph-grid" :class="'g'+Math.min(previews.length,4)">
                        <template x-for="(src,i) in previews.slice(0,4)" :key="i">
                            <div class="ph-thumb" :class="previews.length===1?'tall':''">
                                <img :src="src" alt="">
                                <button type="button" class="ph-remove" @click.prevent="rmImage(i)">×</button>
                            </div>
                        </template>
                    </div>
                </div>
            ...
            @if(in_array($userLevel,['Creator','Influencer']))
                ...
                <div class="ph-vzone"
                    ...
                    <div wire:loading wire:target="video" style="margin-top:12px;font-size:13px;color:var(--ph-sub)">
                        ...
                        Staging file… please wait
                    </div>
                </div>

                {{--
                    ── STEP 2: Ready — file staged in Livewire temp ──────────
                    The file is on the server in temp storage.
                    Show filename + explicit "Upload to Cloudinary" button.
                    This button triggers uploadToCloudinary() in its own
                    separate HTTP request — no timeout from the upload.
                --}}
                <div class="ph-vready"
                     x-show="mode==='video' && vStatus==='ready'"
                     x-cloak>
                    <div class="ph-vready-title">
                        ✅ File ready
                    </div>
                    ...
                    <button type="button"
                            class="ph-upload-btn"
                            wire:click="uploadToCloudinary"
                            wire:loading.attr="disabled"
                            wire:target="uploadToCloudinary">
                        <span wire:loading.remove wire:target="uploadToCloudinary">
                    ...
                    @if($cloudinaryVideoUrl)
                        <div class="ph-vthumb">
                            ...
                            <button type="button" class="ph-vthumb-change"
                                    @click="resetVideo()">Change video</button>
                        </div>
                    @endif
                    <div class="ph-done">
                ...
                <div class="ph-err" x-show="mode==='video' && vStatus==='error'" x-cloak>
                    ...
                    <button type="button" @click="resetVideo()">Try again</button>
                </div>
...
Alpine.data('composerUI', () => ({
    ...
    mode:     'none',   // 'none' | 'image' | 'video'
    previews: [],

    // Video states mirror PHP: '' | 'idle' | 'ready' | 'uploading' | 'done' | 'error'
    vStatus: 'idle',
    vPct:    0,
    vErr:    '',
    ...
    boot() {
        /*
         * Listen for events dispatched by PHP.
         * 'ready'     → file staged, show Upload button
         * 'uploading' → Cloudinary in progress
         * 'done'      → URL stored, show Publish
         * 'error'     → show error + retry
         */
        Livewire.on('videoUploadStatus', ({ status, progress }) => {
            if (status === 'ready')     { this.vStatus = 'ready'; }
    ...
    rmImage(i) {
        ...
        const remaining = Array.from(this.$refs.imgInput.files || []).filter((_, idx) => idx !== i);
        const dt = new DataTransfer();
     ...
     * We ONLY do a client-side duration check here.
     * We do NOT set vStatus to 'uploading' — that would be misleading
     * because the Cloudinary upload hasn't started yet.
     * 
     * wire:model handles the Livewire temp upload in the background.
     * Once Livewire temp upload finishes, PHP's updatedVideo() fires,
     * validates the file, and sets videoUploadStatus = 'ready'.
     * The Livewire.on('videoUploadStatus') listener above will then
     * set vStatus = 'ready' and the Upload button appears.
     *
     * NOTE: we listen for 'ready' from PHP, not set it here.
     * The PHP side dispatches it after successful validation.
     */
    onVideoSelected(e) {
        ...
        tmp.onloadedmetadata = () => {
            ...
            if (tmp.duration > maxSecs) {
                this.vStatus = 'error';
                this.vErr    = `Video is ${Math.round(tmp.duration)}s long. Your limit is ${maxSecs}s.`;
                this.$refs.vidInput.value = '';
                return;
            }
            // Duration OK. Show a "staging" indicator.
            // vStatus stays 'idle' until PHP confirms 'ready'.
            // The wire:loading on the file input shows the spinner.
        };
        tmp.onerror = () => {
            URL.revokeObjectURL(objUrl);
    ...
    onDrop(e) {
        ...
        this.onVideoSelected({ target: this.$refs.vidInput });
    },
    ...
    resetVideo() {
        ...
        // Tell PHP to clear its state too
        Livewire.dispatch('cancelVideoUpload');
    },
