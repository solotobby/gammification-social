    ...
    <div class="content p-0">
        <div class="dash">
            <header class="dash-header">
                <div>
                    <h1>New Help article</h1>
                    <p>Add an answer to the Help Center</p>
                </div>
                <a href="{{ route('admin.help.index') }}" class="dash-btn dash-btn--ghost">
            ...
            <div class="dash-card">
                ...
                <div class="dash-card__body">
                    <form action="{{ route('admin.help.store') }}" method="POST" class="dash-form">
                        @csrf
                        ...
                        <div class="dash-grid-2">
                            <div class="dash-field">
                                ...
                                <select id="help_category_id" name="help_category_id" class="dash-select" required>
                                    <option value="">Select category</option>
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}" @selected(old('help_category_id') == $category->id)>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="dash-field">
                                <label for="sort_order">Sort order</label>
                                <input type="number" id="sort_order" name="sort_order" class="dash-input" min="0" max="9999" value="{{ old('sort_order', 0) }}">
                            </div>
                        </div>
                        ...
                        <div class="dash-field">
                            <label for="js-ckeditor">Answer / body</label>
                        ...
                        <div>
                            <button type="submit" class="dash-btn dash-btn--primary">
                                <i class="fa fa-check"></i> Create Help article
                            </button>
                        </div>
                    </form>
                </div>
