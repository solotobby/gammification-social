...
class GeneralController extends Controller
{
    ...
    public function test()
    {
        ...
        $endDate   = now()->subDay()->toDateString(); // yesterday
        ...
        foreach ($activeUsers as $userLevel) {
            ...
            DB::transaction(function () use ($userLevel, $startDate, $endDate) {

                // Prevent duplicate monthly-to-date record for yesterday
                if (FremiumEngagementStat::where('user_id', $userLevel->user_id)
                    ->where('date', $endDate)
                    ->exists()
                ) {
                    return;
                ...
                FremiumEngagementStat::create([
                    'user_id'  => $userLevel->user_id,
                    'level'    => $userLevel->plan_name,
                    'date'     => $endDate, // snapshot date (yesterday)
                    'views'    => $views,
                    'likes'    => $likes,
                    'comments' => $comments,
                    'points'   => $points,
        ...
        Mail::to('solotob3@gmail.com')->send(
            new GeneralMail(
                (object)[
                    'name' => 'Oluwatobi Solomon',
                    'email' => 'solotob3@gmail.com'
                ],
                'Freemium Monthly-to-Date Engagement Registered',
                'Freemium engagement stats (from start of month to yesterday) registered successfully.'
            )
        );
