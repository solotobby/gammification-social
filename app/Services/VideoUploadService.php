...
class VideoUploadService
{
    ...
    /**
     * Synchronous path — stage + process in one call (legacy / tests).
     */
        ...
        try {
            ...
            foreach ($variants as $label => $cfg) {
                ...
                if ($this->transcodeVariant($master, $local, $cfg, $maxSeconds)) {
                    $key = $prefix.'/'.$cfg['file'];
                    $disk->put($key, fopen($local, 'r'), ['visibility' => 'public', 'ContentType' => 'video/mp4']);
            ...
            if ($this->extractPosterWebp($master, $posterLocal)) {
                ...
                $disk->put($posterKey, fopen($posterLocal, 'r'), ['visibility' => 'public', 'ContentType' => 'image/webp']);
                ...
                if ($this->extractPosterJpg($master, $posterJpg)) {
                    ...
                    $disk->put($posterKey, fopen($posterJpg, 'r'), ['visibility' => 'public', 'ContentType' => 'image/jpeg']);
                    $thumbnail = $this->spacesUrl($posterKey);
                }
            }
